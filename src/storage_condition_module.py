...
@dataclass
class StorageTrainConfig:
    data_dir: str
    output_dir: str = "storage_condition_runs"
    ...
    min_images_per_storage_class: int = 8
    min_storage_classes_per_fruit: int = 2

    rejection_keep_correct_rate: float = 0.95
    model_version: str = "storage_condition_v2"
...
def build_storage_manifest(data_dir: str) -> pd.DataFrame:
    """
    Expected structure:
    data_dir/
        <routing_fruit>/
    ...
    for fruit_dir in fruit_dirs:
        ...
        storage_dirs = sorted([p for p in fruit_dir.iterdir() if p.is_dir()], key=lambda x: x.name)

        for storage_dir in storage_dirs:
            storage_condition = storage_dir.name
    ...
    if manifest.empty:
        raise ValueError("No valid image files found in storage-condition dataset.")
...
def validate_manifest(
    ...
    min_images_per_storage_class: int,
    min_storage_classes_per_fruit: int,
) -> pd.DataFrame:
    counts = (
        manifest.groupby(["routing_fruit", "storage_condition"])
        .size()
        .reset_index(name="count")
    )

    valid_pairs = counts[counts["count"] >= min_images_per_storage_class].copy()
    if valid_pairs.empty:
        raise ValueError("No fruit-storage groups meet minimum sample threshold.")

    valid_class_counts = (
        valid_pairs.groupby("routing_fruit")["storage_condition"]
        .nunique()
        .reset_index(name="num_storage_classes")
    )

    valid_fruits = valid_class_counts[
        valid_class_counts["num_storage_classes"] >= min_storage_classes_per_fruit
    ]["routing_fruit"].tolist()

    filtered = manifest.merge(
        valid_pairs[["routing_fruit", "storage_condition"]],
        on=["routing_fruit", "storage_condition"],
        how="inner",
    )
...
def make_storage_label_mapping(df: pd.DataFrame) -> Tuple[Dict[str, int], Dict[int, str]]:
    class_names = sorted(df["storage_condition"].unique().tolist())
...
def build_storage_classifier(
    ...
    return tf.keras.Model(inputs=inputs, outputs=outputs, name="storage_condition_classifier")
...
def train_single_fruit_model(
    ...
    cfg: StorageTrainConfig,
    root_output_dir: Path,
) -> Dict:
    class_to_idx, idx_to_class = make_storage_label_mapping(fruit_df)
    fruit_df = fruit_df.copy()
    fruit_df["label"] = fruit_df["storage_condition"].map(class_to_idx)
    ...
    model = build_storage_classifier(
    ...
    compile_classifier(model, cfg.learning_rate_stage1, len(class_to_idx))
    model.fit(
        train_ds,
    ...
    compile_classifier(model, cfg.learning_rate_stage2, len(class_to_idx))
    model.fit(
        train_ds,
        validation_data=val_ds,
    ...
    summary = {
        ...
        "num_storage_classes": len(class_to_idx),
        "top1_accuracy": top1,
    ...
    metadata = {
        "module_name": "storage_condition_classification",
        "routing_fruit": routing_fruit,
        "fruit_slug": fruit_slug,
        "model_version": cfg.model_version,
        "image_size": list(cfg.image_size),
        "num_storage_classes": len(class_to_idx),
        "storage_to_idx": class_to_idx,
        "idx_to_storage": {str(v): k for k, v in class_to_idx.items()},
        "rejection_threshold": rejection_threshold,
    }
    ...
    return {
        ...
        "num_storage_classes": int(len(class_to_idx)),
...
@dataclass
class TopKStoragePrediction:
    storage_condition: str
    confidence: float


@dataclass
class StorageConditionOutput:
    module_name: str
    model_version: str
    ...
    predicted_storage_condition: Optional[str]
    ...
    top_k: List[TopKStoragePrediction]
...
class StorageConditionService:
    ...
    def predict(
        ...
        if general_rejected or routing_fruit is None:
            return StorageConditionOutput(
                ...
                predicted_storage_condition=None,
        ...
        idx_to_storage = {int(k): v for k, v in metadata["idx_to_storage"].items()}
        pred_storage = idx_to_storage[pred_idx]

        top_indices = np.argsort(probs)[-top_k:][::-1]
        top_predictions = [
            TopKStoragePrediction(
                storage_condition=idx_to_storage[int(i)],
        ...
        return StorageConditionOutput(
            ...
            quality_label=quality_label,
            ripeness_stage=ripeness_stage,
            predicted_storage_condition=None if rejected else pred_storage,
    ...
    def to_pipeline_features(self, output: StorageConditionOutput) -> Dict[str, object]:
        return {
            "storage_routing_fruit": output.routing_fruit,
            "storage_variety_name": output.variety_name,
            "storage_quality_label": output.quality_label,
            "storage_ripeness_stage": output.ripeness_stage,
            "storage_predicted_class": output.predicted_storage_condition,
            "storage_predicted_index": output.predicted_index,
            "storage_confidence": output.confidence,
            "storage_rejected": output.rejected,
            "storage_skipped": output.skipped,
            "storage_probability_vector": output.probability_vector,
        }
...
def run_training(cfg: StorageTrainConfig) -> None:
    set_seed(cfg.seed)

    output_dir = Path(cfg.output_dir)
    ensure_dir(output_dir)

    manifest = build_storage_manifest(cfg.data_dir)
    manifest = validate_manifest(
        manifest,
        min_images_per_storage_class=cfg.min_images_per_storage_class,
        min_storage_classes_per_fruit=cfg.min_storage_classes_per_fruit,
    )
    ...
    for routing_fruit in routing_fruits:
        ...
        registry["fruit_models"][routing_fruit] = {
            ...
            "num_storage_classes": result["num_storage_classes"],
            "storage_classes": result["storage_classes"],
        }

    pd.DataFrame(fruit_summaries).to_csv(output_dir / "training_summary.csv", index=False)
    save_json(registry, output_dir / "storage_registry.json")

    print(f"Storage-condition training complete. Registry: {output_dir / 'storage_registry.json'}")


if __name__ == "__main__":
    cfg = StorageTrainConfig(
        data_dir=r"C:\Data\DataSetSmallV1\Storage Condition",
        output_dir="storage_condition_runs",
        image_size=(260, 260),
        batch_size=24,
        model_version="storage_condition_v2",
    )
