...
@dataclass
class VarietyTrainConfig:
    data_dir: str
    output_dir: str = "variety_classification_runs"
    ...
    fine_tune_at: int = 250

    min_images_per_variety: int = 8
    min_varieties_per_fruit: int = 2

    rejection_keep_correct_rate: float = 0.95
    model_version: str = "variety_classification_v2"
...
def build_variety_manifest(data_dir: str) -> pd.DataFrame:
    """
    Expected structure:
    data_dir/
        <routing_fruit>/
    ...
    for fruit_dir in fruit_dirs:
        ...
        variety_dirs = sorted([p for p in fruit_dir.iterdir() if p.is_dir()], key=lambda x: x.name)
        for variety_dir in variety_dirs:
            variety_name = variety_dir.name

            for file_path in variety_dir.rglob("*"):
    ...
    if manifest.empty:
        raise ValueError("No valid image files found in variety dataset.")
...
def validate_manifest(
    ...
    min_images_per_variety: int,
    min_varieties_per_fruit: int,
) -> pd.DataFrame:
    counts = (
        manifest.groupby(["routing_fruit", "variety_name"])
        .size()
        .reset_index(name="count")
    )

    valid_pairs = counts[counts["count"] >= min_images_per_variety].copy()
    if valid_pairs.empty:
        raise ValueError("No fruit-variety groups meet minimum sample threshold.")

    valid_variety_counts = (
        valid_pairs.groupby("routing_fruit")["variety_name"]
        .nunique()
        .reset_index(name="num_varieties")
    )

    valid_fruits = valid_variety_counts[
        valid_variety_counts["num_varieties"] >= min_varieties_per_fruit
    ]["routing_fruit"].tolist()

    filtered = manifest.merge(
        valid_pairs[["routing_fruit", "variety_name"]],
        on=["routing_fruit", "variety_name"],
        how="inner",
    )
...
def make_variety_label_mapping(df: pd.DataFrame) -> Tuple[Dict[str, int], Dict[int, str]]:
    variety_names = sorted(df["variety_name"].unique().tolist())
    variety_to_idx = {name: i for i, name in enumerate(variety_names)}
    idx_to_variety = {i: name for name, i in variety_to_idx.items()}
    return variety_to_idx, idx_to_variety
...
def build_variety_classifier(
    ...
    return tf.keras.Model(inputs=inputs, outputs=outputs, name="variety_classifier")
...
def train_single_fruit_model(
    ...
    cfg: VarietyTrainConfig,
    root_output_dir: Path,
) -> Dict:
    variety_to_idx, idx_to_variety = make_variety_label_mapping(fruit_df)
    fruit_df = fruit_df.copy()
    fruit_df["label"] = fruit_df["variety_name"].map(variety_to_idx)
    ...
    model = build_variety_classifier(
        num_classes=len(variety_to_idx),
    ...
    compile_classifier(model, cfg.learning_rate_stage1, len(variety_to_idx))
    model.fit(
        train_ds,
    ...
    compile_classifier(model, cfg.learning_rate_stage2, len(variety_to_idx))
    model.fit(
        train_ds,
        validation_data=val_ds,
    ...
    class_names = [idx_to_variety[i] for i in range(len(idx_to_variety))]
    ...
    summary = {
        ...
        "num_varieties": len(variety_to_idx),
        "top1_accuracy": top1,
    ...
    metadata = {
        "module_name": "variety_classification",
        "routing_fruit": routing_fruit,
        "fruit_slug": fruit_slug,
        "model_version": cfg.model_version,
        ...
        "idx_to_variety": {str(v): k for k, v in variety_to_idx.items()},
        "rejection_threshold": rejection_threshold,
    }
    ...
    return {
        ...
        "num_varieties": int(len(variety_to_idx)),
        "varieties": class_names,
...
@dataclass
class TopKVarietyPrediction:
    variety_name: str
    confidence: float


@dataclass
class VarietyClassificationOutput:
    module_name: str
    model_version: str
    routing_fruit: Optional[str]
    predicted_variety: Optional[str]
    ...
    top_k: List[TopKVarietyPrediction]
...
class VarietyClassifierService:
    ...
    def predict(
        ...
        if general_rejected or routing_fruit is None:
            return VarietyClassificationOutput(
                module_name="variety_classification",
        ...
        idx_to_variety = {int(k): v for k, v in metadata["idx_to_variety"].items()}
        pred_variety = idx_to_variety[pred_idx]

        top_indices = np.argsort(probs)[-top_k:][::-1]
        top_predictions = [
            TopKVarietyPrediction(
                variety_name=idx_to_variety[int(i)],
        ...
        return VarietyClassificationOutput(
            ...
            routing_fruit=routing_fruit,
            predicted_variety=None if rejected else pred_variety,
    ...
    def to_pipeline_features(self, output: VarietyClassificationOutput) -> Dict[str, object]:
        return {
            "variety_routing_fruit": output.routing_fruit,
            "variety_predicted_class": output.predicted_variety,
            "variety_predicted_index": output.predicted_index,
            "variety_confidence": output.confidence,
            "variety_rejected": output.rejected,
            "variety_skipped": output.skipped,
            "variety_probability_vector": output.probability_vector,
        }
...
def run_training(cfg: VarietyTrainConfig) -> None:
    set_seed(cfg.seed)

    output_dir = Path(cfg.output_dir)
    ensure_dir(output_dir)

    manifest = build_variety_manifest(cfg.data_dir)
    manifest = validate_manifest(
        manifest=manifest,
        min_images_per_variety=cfg.min_images_per_variety,
        min_varieties_per_fruit=cfg.min_varieties_per_fruit,
    )
    ...
    for routing_fruit in routing_fruits:
        ...
        registry["fruit_models"][routing_fruit] = {
            ...
            "num_varieties": result["num_varieties"],
            "varieties": result["varieties"],
        }

    pd.DataFrame(fruit_summaries).to_csv(output_dir / "training_summary.csv", index=False)
    save_json(registry, output_dir / "variety_registry.json")

    print(f"Variety training complete. Registry: {output_dir / 'variety_registry.json'}")


if __name__ == "__main__":
    cfg = VarietyTrainConfig(
        data_dir=r"C:\Data\DataSetSmallV1\Variety Classification",
        output_dir="variety_classification_runs",
        image_size=(224, 224),
        batch_size=24,
        model_version="variety_classification_v2",
    )
